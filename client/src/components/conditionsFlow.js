import React from "react";
import DragHere from "./dropHere.js";
import "../assets/css/conditionsFlow.css";

function ConditionsFlow(props) {
  return (
    <section className="_149ypsc">
      <nav className="_19r6nlc">
        <i className="_1beb9nj">
          <svg width={7} height={17} xmlns="http://www.w3.org/2000/svg">
            <path
              d="M4 6.965v9.285H3V6.964a3.5 3.5 0 1 1 1 0zM3.5 6a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5z"
              fill="#FFF"
              fillRule="evenodd"
            />
          </svg>
        </i>
        <div className="_6xkn4r">
          <div className="_1c0tdlg">
            <i className="_1vryo1u">
              <svg
                width="31px"
                height="3px"
                viewBox="0 0 31 3"
                version="1.1"
                xmlns="http://www.w3.org/2000/svg"
                xmlnsXlink="http://www.w3.org/1999/xlink"
              >
                <title>icon-branch-center</title>
                <desc>Created with Sketch.</desc>
                <defs />
                <g
                  id="Page-1"
                  stroke="none"
                  strokeWidth={1}
                  fill="none"
                  fillRule="evenodd"
                >
                  <g
                    id="icon-condition-copy-4"
                    transform="translate(-139.000000, -50.000000)"
                    fill="#52525B"
                  >
                    <g
                      id="icon-branch-center"
                      transform="translate(139.000000, 50.000000)"
                    >
                      <g id="center">
                        <rect id="filler" x={0} y={0} width={31} height={3} />
                      </g>
                    </g>
                  </g>
                </g>
              </svg>
            </i>
            <i className="_53vmlf" title="Condense">
              <svg
                width="58px"
                height="25px"
                viewBox="0 0 58 25"
                version="1.1"
                xmlns="http://www.w3.org/2000/svg"
                xmlnsXlink="http://www.w3.org/1999/xlink"
              >
                <g
                  id="Page-1"
                  stroke="none"
                  strokeWidth={1}
                  fill="none"
                  fillRule="evenodd"
                >
                  <g id="icon-branch-condense" fillRule="nonzero">
                    <rect
                      id="Rectangle-2"
                      fillOpacity="0.01"
                      fill="#D8D8D8"
                      x={0}
                      y={0}
                      width={58}
                      height={25}
                    />
                    <path
                      d="M22.881,13 L19.941,10.344 C19.8351165,10.268052 19.7716394,10.1462947 19.77,10.016 C19.77,9.943 19.784,9.875 19.813,9.812 C19.8416404,9.74962782 19.8861158,9.69584362 19.942,9.656 C19.9887563,9.60335424 20.04786,9.56314997 20.114,9.539 C20.183,9.513 20.251,9.5 20.32,9.5 C20.3879987,9.5 20.4553033,9.51367587 20.518,9.54 C20.5805548,9.56557396 20.6363433,9.60527621 20.681,9.656 L24.995,13.5 L20.715,17.344 C20.6072618,17.4432278 20.4664661,17.4988331 20.32,17.5 C20.251,17.5 20.185,17.487 20.122,17.46 C20.0557451,17.4320301 19.9948427,17.3927819 19.942,17.344 C19.8859802,17.3038803 19.8414931,17.2497371 19.813,17.187 C19.7852993,17.1285142 19.7706279,17.0647111 19.77,17 C19.77,16.937 19.784,16.875 19.813,16.812 C19.8416404,16.7496278 19.8861158,16.6958436 19.942,16.656 L22.881,14 L22.881,13 Z M35.114,13 L35.114,14 L38.053,16.656 C38.1088842,16.6958436 38.1533596,16.7496278 38.182,16.812 C38.211,16.875 38.225,16.937 38.225,17 C38.2243721,17.0647111 38.2097007,17.1285142 38.182,17.187 C38.1535069,17.2497371 38.1090198,17.3038803 38.053,17.344 C38.0001573,17.3927819 37.9392549,17.4320301 37.873,17.46 C37.81,17.487 37.744,17.5 37.675,17.5 C37.5285339,17.4988331 37.3877382,17.4432278 37.28,17.344 L33,13.5 L37.314,9.656 C37.3586567,9.60527621 37.4144452,9.56557396 37.477,9.54 C37.5396967,9.51367587 37.6070013,9.5 37.675,9.5 C37.744,9.5 37.812,9.513 37.881,9.539 C37.94714,9.56314997 38.0062437,9.60335424 38.053,9.656 C38.1088842,9.69584362 38.1533596,9.74962782 38.182,9.812 C38.211,9.875 38.225,9.943 38.225,10.016 C38.2233606,10.1462947 38.1598835,10.268052 38.054,10.344 L35.114,13 Z"
                      id="Shape"
                      fill="#FFFFFF"
                    />
                  </g>
                </g>
              </svg>
            </i>
          </div>
          <div className="_3ogmth">
            {props.steps.map((step, id) =>
              id === 0 ? (
                <section className="_bp34sw" key={id}>
                  <div className="_1iaan0e">
                    <i className="_67zturf">
                      <svg
                        width="18px"
                        height="30px"
                        viewBox="0 0 18 30"
                        version="1.1"
                        xmlns="http://www.w3.org/2000/svg"
                        xmlnsXlink="http://www.w3.org/1999/xlink"
                      >
                        <title>icon-branch-left</title>
                        <desc>Created with Sketch.</desc>
                        <defs />
                        <g
                          id="Page-1"
                          stroke="none"
                          strokeWidth={1}
                          fill="none"
                          fillRule="evenodd"
                        >
                          <g
                            id="icon-condition-copy-4"
                            transform="translate(0.000000, -50.000000)"
                          >
                            <g
                              id="icon-branch-left"
                              transform="translate(0.000000, 50.000000)"
                            >
                              <polygon
                                id="filler"
                                fill="#52525B"
                                points="18 8.84261544 18 30 0 30 0 0 10.0495315 0 10.0495315 8.84261544"
                              />
                              <g
                                id="icon-branch-bottom"
                                transform="translate(5.000000, 1.000000)"
                                fill="#FFFFFF"
                              >
                                <path
                                  d="M5,13.746 L7.656,10.806 C7.73194799,10.7001165 7.85370528,10.6366394 7.984,10.635 C8.057,10.635 8.125,10.649 8.188,10.678 C8.25037218,10.7066404 8.30415638,10.7511158 8.344,10.807 C8.39664576,10.8537563 8.43685003,10.91286 8.461,10.979 C8.487,11.048 8.5,11.116 8.5,11.185 C8.49992101,11.2529987 8.48632413,11.3203033 8.46,11.383 C8.43442604,11.4455548 8.39472379,11.5013433 8.344,11.546 L4.5,15.86 L0.656,11.58 C0.55677225,11.4722618 0.50116685,11.3314661 0.5,11.185 C0.5,11.116 0.513,11.05 0.54,10.987 C0.567969935,10.9207451 0.607218128,10.8598427 0.656,10.807 C0.696119689,10.7509802 0.750262857,10.7064931 0.813,10.678 C0.871485789,10.6502993 0.935288915,10.6356279 1,10.635 C1.063,10.635 1.125,10.649 1.188,10.678 C1.25037218,10.7066404 1.30415638,10.7511158 1.344,10.807 L4,13.746 L4,0 L5,0 L5,13.746 Z"
                                  id="Shape"
                                />
                              </g>
                              <path
                                d="M10,0 L10,10 L9,10 L9,1 C9,0.44771525 9.44771525,-1.01453063e-16 10,0 Z"
                                id="left-part"
                                fill="#FFFFFF"
                              />
                            </g>
                          </g>
                        </g>
                      </svg>
                    </i>
                  </div>
                  <div className="_49x36f">
                    <div className="_1f2sk2k">
                      <div className="_1wm26yt" draggable="true">
                        <div className="_1jzahh6">
                          <div className="_onua9m">
                            <div className="_1rf9dyl1">
                              <i>
                                <svg
                                  width="32px"
                                  height="32px"
                                  viewBox="0 0 32 32"
                                  version="1.1"
                                  xmlns="http://www.w3.org/2000/svg"
                                  xmlnsXlink="http://www.w3.org/1999/xlink"
                                >
                                  <g
                                    id="Page-1"
                                    stroke="none"
                                    strokeWidth={1}
                                    fill="none"
                                    fillRule="evenodd"
                                  >
                                    <g
                                      id="icon-condition-center-copy"
                                      fill="#FFFFFF"
                                      fillRule="nonzero"
                                    >
                                      <g id="center">
                                        <path
                                          d="M2.56,16 L16,29.44 L29.44,16 L16,2.56 L2.56,16 Z M0,16 L16,0 L32,16 L16,32 L0,16 Z"
                                          id="Rectangle-3"
                                        />
                                        <path
                                          d="M13.4,13.4 L14.4653658,13.4 L14.4653658,19.64 L13.4,19.64 L13.4,13.4 Z M15.7759349,13.4 L19.64,13.4 L19.64,14.4047458 L16.8413008,14.4047458 L16.8413008,16.0440678 L19.4793496,16.0440678 L19.4793496,17.0488136 L16.8413008,17.0488136 L16.8413008,19.64 L15.7759349,19.64 L15.7759349,13.4 Z"
                                          id="IF"
                                        />
                                      </g>
                                    </g>
                                  </g>
                                </svg>
                              </i>
                              <span className="_fmaty5">A </span>
                            </div>
                          </div>
                        </div>
                        <div className="_1ttegq05" />
                      </div>
                      <DragHere
                        handleDrop={props.handleDrop}
                        stepId={step.step_id}
                        stepChild={step.children}
                      />
                    </div>
                  </div>
                </section>
              ) : (
                <section className="_bp34sw" key={id}>
                  <div className="_1xb7htq">
                    <i className="_67zturf">
                      <svg
                        width="18px"
                        height="30px"
                        viewBox="0 0 18 30"
                        version="1.1"
                        xmlns="http://www.w3.org/2000/svg"
                        xmlnsXlink="http://www.w3.org/1999/xlink"
                      >
                        <title>icon-branch-right</title>
                        <desc>Created with Sketch.</desc>
                        <defs />
                        <g
                          id="Page-1"
                          stroke="none"
                          strokeWidth={1}
                          fill="none"
                          fillRule="evenodd"
                        >
                          <g
                            id="icon-condition-copy-4"
                            transform="translate(-300.000000, -50.000000)"
                          >
                            <g
                              id="icon-branch-right"
                              transform="translate(300.000000, 50.000000)"
                            >
                              <polygon
                                id="filler"
                                fill="#52525B"
                                points="8.071866 0 18 0 18 30 0 30 0 8.88896523 8.071866 8.88896523"
                              />
                              <path
                                d="M8,0 L8,0 C8.55228475,-1.01453063e-16 9,0.44771525 9,1 L9,10 L8,10 L8,0 Z"
                                id="right-part"
                                fill="#FFFFFF"
                              />
                              <path
                                d="M9,14.746 L11.656,11.806 C11.731948,11.7001165 11.8537053,11.6366394 11.984,11.635 C12.057,11.635 12.125,11.649 12.188,11.678 C12.2503722,11.7066404 12.3041564,11.7511158 12.344,11.807 C12.3966458,11.8537563 12.43685,11.91286 12.461,11.979 C12.487,12.048 12.5,12.116 12.5,12.185 C12.499921,12.2529987 12.4863241,12.3203033 12.46,12.383 C12.434426,12.4455548 12.3947238,12.5013433 12.344,12.546 L8.5,16.86 L4.656,12.58 C4.55677225,12.4722618 4.50116685,12.3314661 4.5,12.185 C4.5,12.116 4.513,12.05 4.54,11.987 C4.56796993,11.9207451 4.60721813,11.8598427 4.656,11.807 C4.69611969,11.7509802 4.75026286,11.7064931 4.813,11.678 C4.87148579,11.6502993 4.93528891,11.6356279 5,11.635 C5.063,11.635 5.125,11.649 5.188,11.678 C5.25037218,11.7066404 5.30415638,11.7511158 5.344,11.807 L8,14.746 L8,1 L9,1 L9,14.746 Z"
                                id="Shape"
                                fill="#FFFFFF"
                              />
                            </g>
                          </g>
                        </g>
                      </svg>
                    </i>
                  </div>
                  <div className="_49x36f">
                    <div className="_1f2sk2k">
                      <div className="_1wm26yt" draggable="true">
                        <div className="_1jzahh6">
                          <div className="_onua9m">
                            <div className="_1rf9dyl1">
                              <i>
                                <svg
                                  width="32px"
                                  height="32px"
                                  viewBox="0 0 32 32"
                                  version="1.1"
                                  xmlns="http://www.w3.org/2000/svg"
                                  xmlnsXlink="http://www.w3.org/1999/xlink"
                                >
                                  <g
                                    id="Page-1"
                                    stroke="none"
                                    strokeWidth={1}
                                    fill="none"
                                    fillRule="evenodd"
                                  >
                                    <g
                                      id="icon-condition-center-copy"
                                      fill="#FFFFFF"
                                      fillRule="nonzero"
                                    >
                                      <g id="center">
                                        <path
                                          d="M2.56,16 L16,29.44 L29.44,16 L16,2.56 L2.56,16 Z M0,16 L16,0 L32,16 L16,32 L0,16 Z"
                                          id="Rectangle-3"
                                        />
                                        <path
                                          d="M13.4,13.4 L14.4653658,13.4 L14.4653658,19.64 L13.4,19.64 L13.4,13.4 Z M15.7759349,13.4 L19.64,13.4 L19.64,14.4047458 L16.8413008,14.4047458 L16.8413008,16.0440678 L19.4793496,16.0440678 L19.4793496,17.0488136 L16.8413008,17.0488136 L16.8413008,19.64 L15.7759349,19.64 L15.7759349,13.4 Z"
                                          id="IF"
                                        />
                                      </g>
                                    </g>
                                  </g>
                                </svg>
                              </i>
                              <span className="_fmaty5">B </span>
                            </div>
                          </div>
                        </div>
                        <div className="_1qbqxqi" />
                      </div>
                      <DragHere
                        handleDrop={props.handleDrop}
                        stepId={step.step_id}
                        stepChild={step.children}
                      />
                    </div>
                  </div>
                </section>
              )
            )}
          </div>
        </div>
      </nav>
    </section>
  );
}
export default ConditionsFlow;
